...
import { LiveKitRoom, useTracks, RoomAudioRenderer, useRoomContext, ReceivedChatMessage, useDataChannel } from '@livekit/components-react';
...
// --- Tipos e Interfaces ---
// O tipo ReceivedChatMessage é reutilizado para manter a estrutura
          ...
          {messages.map((msg, index) => {
            ...
            return (
              <div key={index} className={cn("w-full flex", isLocalUser ? 'justify-end' : 'justify-start')}>
                <div className={cn("av-message-bubble p-3 rounded-xl max-w-[85%] text-sm", isLocalUser ? 'bg-gray-800 text-white rounded-br-none' : 'bg-accent text-black rounded-tl-none')}>
                  {msg.message}
                </div>
              </div>
            );
...
const VoiceSessionUI: React.FC<VoiceSessionUIProps> = ({ onConnectionStatusChange }) => {
  ...
  const addMessage = (message: UnifiedMessage) => {
    setAllMessages(prev => [...prev, message].sort((a, b) => a.timestamp - b.timestamp));
  };

  // Ouve por mensagens de texto do agente e transcrições
  useDataChannel('lk-chat-topic', (msg) => {
    try {
      const data = JSON.parse(textDecoder.decode(msg.payload));
      if (data.text) {
        addMessage({
    ...
    } catch (e) { console.error("Erro ao processar mensagem de chat:", e); }
  });

  useDataChannel('user_transcript', (msg) => {
    try {
      const data = JSON.parse(textDecoder.decode(msg.payload));
      if (data.text) {
        addMessage({
          id: Math.random().toString(),
          timestamp: Date.now(),
          message: `"${data.text}"`, // Adiciona aspas para diferenciar transcrições
          from: msg.from,
        });
      }
    } catch (e) { console.error("Erro ao processar transcrição:", e); }
  });
  ...
  const handleVoiceToggle = useCallback(async () => {
    ...
    const command = nextVoiceState ? "activate_voice" : "deactivate_voice";
    const payload = textEncoder.encode(JSON.stringify({ command }));
    await room.localParticipant.publishData(payload, 'agent_control', { reliable: true });
    console.log(`Comando '${command}' enviado no tópico 'agent_control'.`);
  ...
  const handleSendMessage = useCallback(async (message: string) => {
    ...
    const payload = textEncoder.encode(JSON.stringify({ text: message }));
    await room.localParticipant.publishData(payload, 'lk-chat-topic', { reliable: true });
  }, [isVoiceEnabled, room]);

  useDataChannel('navigation_command', (msg) => {
    try {
      const data = JSON.parse(textDecoder.decode(msg.payload));
      if (data.navigateTo) { scrollToSection(data.navigateTo); }
    } catch (e) {}
  });

  const tracks = useTracks([Track.Source.Audio]);
  useEffect(() => {
    if (tracks.some(trackRef => !trackRef.participant.isLocal)) {
